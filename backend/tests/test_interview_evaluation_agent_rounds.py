...
def test_evaluate_logs_current_round_topic_counts(caplog):
    payload = InterviewEvaluationAgentInput.model_validate(
        {
            "sessionId": "session-2",
            "jdText": "负责算法研究",
    ...
    built_inputs = next(record for record in records if record.message == "interview evaluation built topic inputs")
    assert getattr(built_inputs, "topic_input_count") == 2
    first_topic = next(record for record in records if record.message == "interview topic evaluation completed")
    assert getattr(first_topic, "elapsed_ms") >= 0
    assert caplog.text.count("interview topic evaluation completed") == 2
    assert "负责算法研究" not in caplog.text
    assert "我做过模型训练项目" not in caplog.text
